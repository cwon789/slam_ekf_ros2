...
from .util import quaternion_to_yaw, plot_cov, angle_between_yaw

def compute_curvature(pts):
    """Compute the curvature of the scan points.
    ...
    ddx = np.gradient(dx)
    ddy = np.gradient(dy)
    curvature = np.abs(ddx * dy - dx * ddy) / np.power(dx**2 + dy**2, 3/2)
    return curvature

def extract_corners(pts, curvature_threshold=0.1):
    """Extract corners based on curvature.

    Args:
        pts (np.ndarray): Points in 2D space.
        curvature_threshold (float): Threshold above which a point is considered a corner.

    Returns:
        np.ndarray: Extracted corner points.
    """
    curvature = compute_curvature(pts)
    corners = pts[curvature > curvature_threshold]
    return corners

def extract_sharp_corners(pts, curvature_threshold=0.1, angle_threshold=np.pi/4):
    """Extract sharp corners based on curvature and angle filtering.

    Args:
        pts (np.ndarray): Points in 2D space.
        curvature_threshold (float): Threshold above which a point is considered for further analysis.
        angle_threshold (float): The angular threshold (in radians) to identify sharp corners. Default is 45 degrees.

    Returns:
        np.ndarray: Extracted sharp corner points.
    """
    curvature = compute_curvature(pts)
    candidate_corners = pts[curvature > curvature_threshold]

    sharp_corners = []

    for i in range(1, len(candidate_corners) - 1):
        # Compute vectors between consecutive points
        v1 = candidate_corners[i] - candidate_corners[i - 1]
        v2 = candidate_corners[i + 1] - candidate_corners[i]
        ...
        # If the angle is close to 90 degrees (within the threshold), consider it a sharp corner
        if np.abs(np.pi / 2 - angle) < angle_threshold:
            sharp_corners.append(candidate_corners[i])

    return np.array(sharp_corners)
...
class SlamEkf(Node):
    def __init__(self):
        ...
        self.alpha = [0.01, 0.01, 0.1, 0.1] # rot/tr/rot/tr
    ...
    #     """callback for lidar scan topic; group lidar points using curvature
    #     and extract corners to serve as the landmarks
    ...
    #     pts = gaussian_filter1d(pts, sigma=1, axis=0) # apply smoothing
    #     self.lidar_pts_fixedframe = pts # store for plotting

    #     # extract corners based on curvature
    #     # corners = extract_corners(pts)
    #     corners = extract_sharp_corners(pts)
    ...
    def scan_callback(self, msg: LaserScan):
        """callback for lidar scan topic; group lidar points using curvature
        and extract sharp corners to serve as the landmarks
        ...
        pt_y = y + rng[keep] * np.sin(yaw + angles[keep])
        pts = np.hstack([pt_x[:, np.newaxis], pt_y[:, np.newaxis]])
        pts = gaussian_filter1d(pts, sigma=0.1, axis=0) # apply smoothing
        ...
        voxel_size = 0.25  # Adjust the voxel size as needed
        downsampled_pcd = pcd.voxel_down_sample(voxel_size=voxel_size)

        # Convert downsampled points back to numpy array
        downsampled_pts = np.asarray(downsampled_pcd.points)[:, :2]  # Keep only x and y for 2D processing
        self.lidar_pts_fixedframe = downsampled_pts # store for plotting

        # Extract sharp corners based on curvature and angle
        corners = extract_sharp_corners(downsampled_pts)
        ...
        for corner in corners:
            ...
            angle = angle_between_yaw(yaw, np.arctan2(dy, dx))
            data.append([np.sqrt(dx**2 + dy**2), angle])
        self.landmark_measurements.data = np.array(data)
        self.landmark_measurements.timestamp = self.get_curr_time()
    ...
    def cluster_pts(self, pts):
        """cluster lidar points using DBSCAN (this method is kept for future extensions)
        ...
        """
        return []  # Clustering is not used in this implementation
    ...
    def odom_to_control(self):
        ...
        Returns:
            u: vector containing the control command extracted from odom
        """
        # unpack odom info
    ...
    def compute_cov_pred(self, J_motion: np.ndarray, J_noise: np.ndarray, Rn: np.ndarray):
        """computes the covariance matrix of the predicted belief

        Args:
            J_motion (np.ndarray): Jacobian of the motion model with respect to state
            J_noise (np.ndarray): Jacobian of the motion model with respect to noise
            Rn (np.ndarray): Covariance matrix of the motion noise
        """
    ...
    def convert_to_fixed_frame(self, landmark_measurements):
        """convert the landmark measurements (range and bearing)
    ...
    def compute_obs(self, landmark_ind, rng):
        """compute the covariance matrix of the predicted
        measurement and other related variables
    ...
    def slam(self):
        ...
        # command translates into the next state
        ...
        for z, l_xy in zip(l_measurement_polar, l_measurement_xy):
            ...
            if j == -1:  # no association found
                ...
                # set j to the new landmark index
...
def main(args=None):
    ...
    ekf_slam = SlamEkf()
    ekf_slam.get_logger().info("EKF SLAM with curvature-based corner detection started!")
    rclpy.spin(ekf_slam)
    rclpy.shutdown()
